...
export function PersonalAIConnector({ sessionId, prompt, callsUsed, onResult }: Props) {
  ...
  const runPrompt = async () => {
    ...
    } finally { setCallsMade((current) => current + 1) }
  }

  const keyReady = apiKey.trim().length >= 20 && model.trim().length > 0
  const exhausted = callsMade >= 2

  return <section className="personal-ai" aria-labelledby="personal-ai-title">
    <header><div><span>PERSONAL API · MEMORY ONLY</span><h3 id="personal-ai-title">내 AI 연결</h3></div><strong>{status === 'connected' ? 'CONNECTED' : `${callsMade}/2 CALLS`}</strong></header>
    <p>개인 키는 연결 확인과 분석 요청에만 사용하며 서버 DB·Evidence·브라우저 저장소에 보관하지 않아. 이 화면을 벗어나거나 새로고침하면 사라져. 프롬프트와 화면에 공개된 합성 관찰은 선택한 AI 제공사로 전송돼.</p>
    ...
    <div className="personal-ai-actions">
      ...
      <button type="button" className="deepseek-call" onClick={runPrompt} disabled={!secureContext || status !== 'connected' || prompt.trim().length < 20 || exhausted}>{status === 'running' ? '프롬프트 분석 중…' : '2. 이 프롬프트로 분석'}</button>
    </div>
    {message && <p className={status === 'connected' ? 'connection-status' : 'inline-error'} role="status">{message}</p>}
    <small>연결 확인은 모델 조회 API만 사용하며 생성 비용을 발생시키지 않아. 분석 비용·쿼터는 개인 제공사 계정에 적용되고 세션당 최대 2회야.</small>
  </section>
}
