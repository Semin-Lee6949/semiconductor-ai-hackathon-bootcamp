...
function DecisionPanel({ scenario, state, onSubmit, busy }: { scenario: Scenario; state: SessionState; onSubmit: (decision: Decision) => Promise<void>; busy: boolean }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (stage.id === 'coach') Object.assign(payload, { prompt, human_check: humanCheck })
  ...
  return (
    <form className="decision-panel" onSubmit={submit}>
      ...
      {stage.id === 'coach' && <>
        ...
        <p className="field-note">이 MVP는 외부 LLM의 답을 대신 만들지 않고, 질문과 사람의 검증 책임을 기록한다.</p>
      </>}
      ...
      <button className="commit" disabled={!choice || busy}>{busy ? '판단 기록 중…' : '판단을 기록하고 다음 스테이션으로'}</button>
    </form>
  )
}
        ...
        <aside className="workbench">
          {session.completed ? (
            <div className="result-panel">
              ...
              <button className="commit" onClick={restart} disabled={busy}>다른 경로로 다시 실험</button>
            </div>
          ) : (
            <DecisionPanel key={scenario.stages[session.stage_index].id} scenario={scenario} state={session} onSubmit={decide} busy={busy}/>
          )}
