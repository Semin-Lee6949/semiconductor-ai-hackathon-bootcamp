...
4. OpenAI·Gemini·Anthropic·DeepSeek와 최소 8회, 최대 15회 후속 질문을 `용어·데이터 이해 → 경쟁 가설 → 반증·누락 점검 → 판단 압축 → PT 심화` 순서로 사용한다. 각 답변은 합성 CSV 수치·해석·가설·반증·불확실성·다음 질문을 연결한다.
...
- Gemini 3 계열은 `thinkingLevel: low`와 4,096 출력 한도를 사용한다. 사고 토큰 때문에 `MAX_TOKENS`로 짧게 잘린 응답은 8,192 한도·`minimal` 수준으로 한 번 자동 재시도하며, UI에는 응답·사고·전체 토큰을 분리 표시한다.
